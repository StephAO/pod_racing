...
class Pod:
    def __init__(self, checkpoints, req_laps):
        self.x, self.y = np.zeros((4)), np.zeros((4))
        self.vx, self.vy = np.zeros((4))
        self.angle = np.zeros((4))
        self.shield = [False] * 4
        self.boost_used = [True] * 4
...
def test():
    ...
    #     best_score = 0
    #     for j in xrange(250):
    checkpoints = [(0,0), (-2500,0)]
    num_laps = 3
    base_pods = [Pod(checkpoints, num_laps), Pod(checkpoints, num_laps), Pod(checkpoints, num_laps), Pod(checkpoints, num_laps)]
    for p in base_pods:
        p.update_pod(0,0,0,0,0,1)

    evol = Evolution(base_pods)
    
    champion, sith_lord, my_gens, enemy_gens = evol.run()

    final_score, x, y = evol.simulate(True, np.concatenate([champion, sith_lord]), extra=True)
